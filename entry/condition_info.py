...
class InfoClass():
    ...
    def explain_conditions(self):
        ...
        self.cond_win.geometry('1000x700')
        style = ThemedStyle(self.cond_win)
        style.theme_use("equilux")
        bg = style.lookup('TLabel', 'background')
        fg = style.lookup('TLabel', 'foreground')
        self.cond_win.configure(bg=style.lookup('TLabel', 'background'))
        self.condition_canvas = tk.Canvas(master=self.cond_win, bg='gray28', width=1000, highlightthickness=0)
        self.condition_canvas.pack(side='top', fill='both', expand=True)
        ...
        info_frame.pack(side='top', pady=15, padx=15)
